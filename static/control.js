...
function setupProgressiveMode() {
  ...
  banner.innerHTML = `
    <div>
      <strong>📄 Weitere Dateien werden im Hintergrund analysiert...</strong>
...
function finishProgressiveMode() {
  ...
  if (banner) {
    ...
    banner.innerHTML = `
      <div>
        <strong>✅ Alle Dateien analysiert!</strong>
...
async function saveData() {
  ...
  // Formular → Objekt (Session-Felder nutzen gesperrte Werte)
  SESSION_FIELDS.forEach(field => {
    if (sessionData[field].locked) {
...
function checkFinalizeReady() {
  ...
  if (allVisited && !allDecided) {
    ...
    console.log(`⚠️ ${remaining} Datei(en) haben noch keine Include-Entscheidung`);
  }
}
...
async function finalizeAnalysis() {
  ...
  if (undecided.length > 0) {
    alert(`⚠️ Bitte treffen Sie für alle ${undecided.length} Datei(en) eine Entscheidung (Ja/Nein), bevor Sie die Analyse abschließen.`);
    return;
  }
